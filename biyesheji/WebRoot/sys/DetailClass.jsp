...
<html>
  <head>
    ...
    <script type="text/javascript">
        function createMajor(v1,optext){
		    if(v1==""){
		      $("s2").options.length=0;
			...
			if(xhr.readyState==4){
					...
					for(i=0;i<majors.length;i++){
						...
						$('s2').options[i+1]=op;
						if(optext==mj.name){
						  $('s2').options[i+1].setAttribute("selected",true);
						}
					}
				}
			};
  ...
  <body>
     ...
     <table width="435" height="323">
        ...
        <tr>
           <td class="td1">学历：</td>
           <td class="td2">
              <select name="cla.edu_bg" id="s1" onchange="createMajor(this.value,'---选择专业---')">
        ...
        <tr>
           <td>
              <input type="hidden" name="cla.id" id="hid"/>
           </td>
           <td>
  ...
  <script type="text/javascript">
	...
	if(obj!=null){
	  //设置打开时窗体中各项的值
	  document.getElementById("grade").value=obj[2].innerHTML.substring(0,2);
	  var op=document.getElementById("s1").options;
	  for(var i=0;i<op.length;i++){
	      if(op[i].value==obj[3].innerHTML){
	         op[i].setAttribute("selected",true);
	      }
	  }
	  
	  var op3=document.getElementById("class_name").options;
	  for(var i=0;i<op3.length;i++){
	      if(op3[i].value==obj[5].innerHTML){
	         op3[i].setAttribute("selected",true);
	      }
	  }
	  
	  document.getElementById("class_abbr").value=obj[6].innerHTML;
	  document.getElementById("hid").value=obj[0].getElementsByTagName("input")[0].value;
	  
	  createMajor(obj[3].innerHTML,obj[4].innerHTML);
   }
