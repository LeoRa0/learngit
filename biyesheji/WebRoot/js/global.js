...
String.prototype.trim = function(){
	...
	try{
		...
		case "comma" : reExtraSpace = /^,*(.*?),*$/; break;
		case "bigcomma" : reExtraSpace = /^，*(.*?)，*$/; break;
		case "dollar" : reExtraSpace = /^\$*(.*?)\$*$/; break;
		default : reExtraSpace = /^\s*(.*?)\s+$/; break;
		}
	}catch(e){
		reExtraSpace = /^\s*(.*?)\s+$/;
	}
...
Date.prototype.dateValue = function(){
	if(this.getFullYear() != 1900){
		var rst = '';
		rst += this.getFullYear()+'-';
		rst += (this.getMonth()+1) < 10 ? '0'+(this.getMonth()+1)+'-' : (this.getMonth()+1)+'-';
		rst += this.getDate() < 10 ? '0'+this.getDate() : this.getDate();
		return rst;
	}else{
		return '';
	}
							   ...
							   if(mode){
								   onreadystatechange = function (){
									   if(objXMLHttp.readyState == 4 && (objXMLHttp.status == 200 || objXMLHttp.status == 304)){
										   ...
										   if(confirm("发生一个错误，单击“确定”立即显示错误信息，单击“取消”转到错误页面")){
